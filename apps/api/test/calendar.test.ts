...
test('tomorrow search → explicit exact-day booking → next candidate; original gap stays pending', async () => {
  ...
  try {
    ...
    const tomorrow = await demo.check(session.id, { date: '2026-09-20', partOfDay: 'afternoon' });
    assert.equal(tomorrow.referenceDate, '2026-09-19');
    assert.ok(tomorrow.availableSlots.length >= 3);
    assert.ok(tomorrow.availableSlots.every(slot => slot.date === '2026-09-20' && slot.time >= '12:00' && slot.time < '17:00'));
    assert.ok(tomorrow.availableSlots.every(slot => !['12:30', '12:45', '13:00', '13:15', '13:30'].includes(slot.time)));
    assert.equal((await demo.check(session.id, { date: '2026-09-20', time: '13:00' })).available, false);
    assert.equal((await demo.check(session.id, { date: '2026-09-20', time: '14:00' })).available, true);
    assert.equal(bookings.bookings().length, 6, 'search never reserves');
    await assert.rejects(demo.accept(session.id, { date: '2026-09-20', time: '14:00' }), /explicit agreement/);
    await assert.rejects(demo.accept(session.id, { date: '2026-09-20', time: '14:00', confirmed: false }), /explicit agreement/);
    await assert.rejects(demo.accept(session.id, { date: '2026-09-20', time: '13:00', confirmed: true }), /unavailable/);
    const request = { date: '2026-09-20', time: '14:00', confirmed: true };
    ...
    assert.match(saved.booking!.spokenDate!, /Sunday, September 20/);
    ...
    assert.ok(bookings.events().some(event => event.message.includes('2026-09-20 at 14:00')));
    await assert.rejects(demo.createSession(), /already active/);
    await assert.rejects(demo.accept(session.id, { ...request, date: '2026-09-19' }), /no longer active/);
...
test('calendar respects duration, hours, horizon, and separates a same-day appointment outside the refill window', async () => {
  ...
  try {
    ...
    const session = await demo.createSession();
    for (const [date, time] of [['2026-09-18', '15:00'], ['2026-09-26', '15:00'], ['2026-09-20', '08:45'], ['2026-09-20', '17:30'], ['2026-09-19', '16:00']]) {
      assert.equal((await demo.check(session.id, { date, time })).available, false, `${date} ${time}`);
    }
    assert.equal((await demo.check(session.id, { date: '2026-09-25', time: '17:15' })).available, true);
    const saved = await demo.accept(session.id, { date: '2026-09-19', time: '11:30', confirmed: true });
    assert.equal(saved.status, 'alternative_booked');
    assert.equal(saved.feeWaived, false);
  } finally { bookings.close(); }
});
...
test('writes recheck collisions and reject reusing a booking key for another date', async () => {
  ...
  try {
    ...
    const input = { slotId: 'slot_3pm', contactId: 'client_5', startsAt: demoTime('14:00', '2026-09-20'), idempotencyKey: 'jordan' };
    ...
    await assert.rejects(bookings.bookReplacement({ ...input, startsAt: demoTime('14:00', '2026-09-21') }), /another booking/);
...
test('tool schemas retain legacy HH:mm and validate dated confirmation and real dates', () => {
  ...
  assert.equal(voiceAvailabilitySchema.safeParse({ date: '2026-09-20', partOfDay: 'afternoon' }).success, true);
  assert.equal(voiceAcceptSchema.safeParse({ time: '15:30' }).success, true);
  assert.equal(voiceAcceptSchema.safeParse({ date: '2026-09-20', time: '14:00' }).success, false);
  assert.equal(voiceAcceptSchema.safeParse({ date: '2026-09-20', time: '14:00', confirmed: true }).success, true);
});
