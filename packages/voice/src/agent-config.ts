...
Before booking, ask the customer to explicitly agree to the exact named day and time. A search request or tentative interest is not consent. Only then call accept_slot with that exact date, time, and confirmed: true. Confirm only after ok: true using the returned booking date and time. A separate appointment on another day does not fill the original opening; never promise a fee waiver for it. End the current call after the confirmed booking as usual.`;
