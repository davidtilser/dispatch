  ...
  const login = (usernameInput: string, passwordInput: string, role: UserRole) => {
    ...
    if (trimmedUser === 'login' && trimmedPass === 'password') {
      const newUser: User = {
        username: 'login',
        role,
        name: role === 'client' ? 'Demo Client' : 'Fade Factory Barber Co.',
      };
      setUser(newUser);
      return { success: true };
    }
    ...
    if ((trimmedUser === 'client' || trimmedUser === 'business' || trimmedUser === 'admin') && trimmedPass === 'password') {
      ...
      const newUser: User = {
        ...
        role: detectedRole,
        name: detectedRole === 'client' ? 'Demo Client' : 'Fade Factory Barber Co.',
      };
      setUser(newUser);
      return { success: true };
    }
  ...
  const switchRole = (newRole: UserRole) => {
    if (user) {
      setUser({
        ...user,
        role: newRole,
        name: newRole === 'client' ? 'Demo Client' : 'Fade Factory Barber Co.',
      });
    }
  };
